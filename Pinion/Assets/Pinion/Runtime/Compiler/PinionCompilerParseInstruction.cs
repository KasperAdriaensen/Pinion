	...
	public partial class PinionCompiler
	{
		...
		private static CompilerArgument ParseInstruction(PinionContainer targetContainer, Token token, List<ushort> output, IList<CompilerArgument> providedArguments)
		{
			...
			if (!fullMatch)
			{
				...
				{
					errorMessage = $"Instruction {instructionString} is not valid within this container type ({targetContainer.GetType()}). It is restricted to a container of type {matchedInstruction.GetExpectedContainerType()}";
				}
